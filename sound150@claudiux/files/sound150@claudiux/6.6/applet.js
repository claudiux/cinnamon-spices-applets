...
const ARTDIR = XDG_RUNTIME_DIR + "/sound150/arts";
...
class Sound150Applet extends Applet.TextIconApplet {
    constructor(metadata, orientation, panel_height, instanceId) {
        ...
        Util.spawnCommandLine("/usr/bin/env bash -c '[[ -d %s ]] || mkdir -p %s'".format(ARTDIR, ARTDIR));
        ...
        this.title_text = "";
        this.tooltipForAdvDetection = "";
        this.tooltipForAdvDetectionOLD = "";
    ...
    constructor_continuation() {
        ...
        this.settings.bind("dontShowAnyImageInMenu", "dontShowAnyImageInMenu", (value) => {
            if (value === true)
                this.showMediaOptical = false;
            SHOW_MEDIA_OPTICAL = this.showMediaOptical;
            this._on_reload_this_applet_pressed();
        });
        SHOW_MEDIA_OPTICAL = this.showMediaOptical;
        ...
        this.settings.bind("showVolumeLevelNearIcon", "showVolumeLevelNearIcon", () => {
            this.volume_near_icon("settings: showVolumeLevelNearIcon")
        });
        ...
        this.settings.bind("adv_old_volume", "adv_old_volume");
        this.settings.bind("adv_icon", "adv_icon");
        this.adv_icon = this.adv_icon.replace("~/", HOME_DIR + "/");
        this.settings.bind("adv_list", "adv_list");
    ...
    unmonitor_icon_dir() {
        ...
        if (! this.iconsMonitor.is_cancelled() )
    ...
    on_applet_added_to_panel() {
        ...
        try {
            if (this.themeNode == null && this.actor._get_stage() != null && this.actor.get_theme_node) {
        ...
        this.artsMonitor = null;
        this.artsMonitorId = null;
        this.monitor_art_dir();

        this.volume_near_icon("on_applet_added_to_panel()");
    ...
    _volumeChange(direction) {
        ...
        if (volumeChange) {
            ...
            intervalId = setTimeout(() => {
                if (intervalId && source_exists(intervalId))
    ...
    setIcon(icon, source, force=false) {
        ...
        if (this.playerControl && this._activePlayer && this._playerIcon && this._playerIcon[0]) {
                ...
                if (this._playerIcon && this._playerIcon[1]) {
                    ...
                    if (this._playerIcon[0] != this.oldPlayerIcon0 || !this._iconTimeoutId || force) {
                        ...
                        if (! baseName.startsWith("R3SongArt")) {
                            const source = Gio.File.new_for_path(this.oldPlayerIcon0);
            ...
            //~ if (! this.keepAlbumArtIcon) this.set_applet_icon_symbolic_name(this._outputIcon);
            this.set_applet_icon_symbolic_name(this._outputIcon);
        }
        this.volume_near_icon("setIcon")
    }
    ...
    set_applet_icon_path(icon_path) {
        ...
        try {
            ...
            let icon_path2 = icon_path.replace("AlbumArt/song-art", "sound150/arts");
            let file;
    ...
    setAppletText(player) {
        ...
        //~ logDebug("this.title_text_old: " + this.title_text_old);
        //~ logDebug("this.title_text: " + this.title_text);
        if (this.title_text_old != this.title_text) {
            //~ logDebug("Title has changed!");
            del_song_arts();
        }
    ...
    setAppletTooltip() {
        ...
        if (this.player && this.player._owner == this._activePlayer) {
            ...
            if (this.adv_avoid && this.adv_list.length > 0) {
                // Manage advertising:
                var advList = [];
                for (let a of this.adv_list) {
                    let adv = a["adv"]
                    if (adv.length > 0)
                        advList.push(adv);
                }
                //~ logDebug("advList: " + advList);
                var wasAdv = false;
                var isAdv = false;
                for (let a of advList) {
                    if (this.tooltipForAdvDetectionOLD.includes(a)) {
                        wasAdv = true;
                    }
                    if (this.tooltipForAdvDetection.includes(a)) {
                        isAdv = true;
                    }
                }
                //~ logDebug("wasAdv: " + wasAdv);
                //~ logDebug("isAdv: " + isAdv);
                if (!wasAdv && isAdv) {
                    // Advert is coming!
                    this.adv_old_volume = this.volume;
                    this.volume = this.adv_volume;
                    this._output.volume = (parseInt(this.volume) / 100) * this._volumeNorm;
                    this._output.push_volume();
                    if (GLib.file_test(this.adv_icon, GLib.FileTest.EXISTS))
                        this.set_applet_icon_path(this.adv_icon);
                }
                if (wasAdv && !isAdv) {
                    // Advert is gone!
                    this.volume = this.adv_old_volume;
                    this._output.volume = (parseInt(this.volume) / 100) * this._volumeNorm;
                    this._output.push_volume();
                    this.setIcon();
                }
            ...
            if (this.tooltipShowArtistTitle) {
                ...
                if (this.player._title != _("Unknown Title")) {
    ...
    _theme_set() {
        ...
        let color = this.color0_100;
        try {
            if (!this.themeNode && this.actor._get_stage() != null && this.actor.get_theme_node) {
    ...
    _onDeviceAdded(control, id, type) {
        ...
        //~ logDebug("_onDeviceAdded: " + device.description);
        var isUnwanted = false;
        var ignoredDevices;
        if (type === "output") {
            ignoredDevices = this.ignoredOutputDevices;
        } else {
            ignoredDevices = this.ignoredInputDevices;
        }
        for (let part of ignoredDevices) {
            if (device.description.includes(part["partOfName"])) {
                isUnwanted = true;
                break;
            }
        }
        //~ logDebug("isUnwanted: " + isUnwanted);
    ...
    volume_near_icon(comesFrom="") {
        ...
        if (this.showMediaKeysOSD &&
            ...
            //~ logDebug("this.old_volume: " + this.old_volume);
        }
        this.alreadyCalledBysetAppletTooltip = comesFrom == "setAppletTooltip()";
        let label = "";
